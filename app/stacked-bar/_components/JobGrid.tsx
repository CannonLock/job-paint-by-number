...
import type { BarCounts } from "./dayCards";
import { BAR_STATE_GLYPH, BAR_STATE_STYLES } from "./palette";
import { BOX_COUNT, GRID_DIM, buildCells } from "./waffle";

interface JobGridProps {
  /** Three-state breakdown of the cohort as of today, or null to render empty. */
  counts: BarCounts | null;
  ...
  gap?: number;
  /** Draw the per-state glyph inside each box; only legible at ~12px and up. */
  showGlyphs?: boolean;
}

/**
 * The 10x10 waffle, three-state edition. A cohort of any size maps onto the same
 * grid, so days stay visually comparable regardless of how many jobs each holds.
 */
